...
class SAN(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        mask_embs, rec_intermediate_features, rec_intermediate_attn_weights = [], [], []
        for attn_bias in attn_biases:
            me, rif, riaw = self.clip_rec_head(clip_image_features, attn_bias, normalize=True)
            mask_embs.append(me)
            rec_intermediate_features.append(rif)
            rec_intermediate_attn_weights.append(riaw)
    ...
    def preprocess(self, batched_inputs):
        ...
        images = ImageList.from_tensors(images, self.size_divisibility)
        clip_input = images.tensor
        return {'clip_input': clip_input}
    
    def attack_forward(self, batched_inputs, attacking=False):
        # get classifier weight for each dataset
        # !! Could be computed once and saved. It will run only once per dataset.
        # print(batched_inputs[0]["vocabulary"])
        if "ov_classifier_weight" in batched_inputs[0]:
            ov_classifier_weight = batched_inputs[0]["ov_classifier_weight"]
        elif "vocabulary" in batched_inputs[0]:
            ov_classifier_weight = (
                self.ov_classifier.logit_scale.exp()
                * self.ov_classifier.get_classifier_by_vocabulary(
                    batched_inputs[0]["vocabulary"]
        ...
        clip_image_features = self.clip_visual_extractor(clip_input)
        # print( clip_image_features[9].shape, clip_image_features['9_cls_token'].shape ) # torch.Size([1, 768, 12, 16]) torch.Size([1, 1, 768])   
        clip_image_features_out = [ clip_image_features[i] for i in range(self.clip_visual_extractor.last_output_idx-1)]
        clip_image_features_out_cls_token = [ clip_image_features[f'{i}_cls_token'] for i in range(self.clip_visual_extractor.last_output_idx-1)]
        # clip_image_features_out_attn_weight = [ clip_image_features[f'{i}_attn_weight'] for i in range(1, self.clip_visual_extractor.last_output_idx-1)]
        # _, intermediate_featuers = self.clip_model.visual(images.tensor, return_cls=False)

        mask_preds, attn_biases, san_features = self.side_adapter_network(
            images.tensor, clip_image_features
        )
        # [print(f["query"].shape) for f in san_features] #[1, 100, 240]
        # [print(f["x"].shape) for f in san_features] #[1, 240, 28, 36]
        # print(attn_biases[0][0].shape) torch.Size([1, len(visual_encoder.transformer.resblocks), 100, H/16, W/16])
        # !! Could be optimized to run in parallel.
        mask_embs, rec_intermediate_features, rec_intermediate_attn_weights = [], [], []
        for attn_bias in attn_biases:
            me, rif, riaw = self.clip_rec_head(clip_image_features, attn_bias, normalize=True)
        ...
        if "instances" in batched_inputs[0]:
            gt_instances = [x["instances"].to(self.device) for x in batched_inputs]
            targets = self.prepare_targets(gt_instances, images)
            # print(targets[0]["masks"].shape) # [n_classes, h, w]
        else:
            targets = None
        outputs = {
                "pred_logits": mask_logits[-1],
        ...
        if self.training or attacking:
            ...
            for k in list(losses.keys()):
                if k in self.criterion.weight_dict:
                    ...
                    # remove this loss if not specified in `weight_dict`
                    losses.pop(k)
        if not attacking and self.training: return losses

        outputs.update(
            {
            "clip_image_features": clip_image_features_out, # [B, 768, H, W]
            "clip_image_features_cls_token": clip_image_features_out_cls_token, # [B, 1, 768]
            # "clip_image_attn_weight": clip_image_features_out_attn_weight, # [B, 1+HW, 1+HW]
            "attn_biases": attn_biases,
            "san_features": san_features, 
            "rec_intermediate_features": rec_intermediate_features,
            "rec_intermediate_attn_weights": rec_intermediate_attn_weights,
            }
        )
        ...
        for mask_cls_result, mask_pred_result, input_per_image, image_size in zip(
            ...
            processed_results[-1]["sem_seg"] = r
        if attacking:
            return losses, outputs, processed_results
        return processed_results
