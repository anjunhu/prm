...
def register_all_pcontext_adv(root):
    root = os.path.join(root, "pcontext")

    for surrogate in ['san', 'cat', 'deop']:
        for strategy in ['pgd', 'latest', 'baseline']:
            for name, dirname in [("train", "train"), ("val", "val")]:
                image_dir = os.path.join(root, dirname, f"image_adv_{surrogate}_{strategy}")
                gt_dir = os.path.join(root, dirname, "label")
                name = f"pcontext_sem_seg_{name}_adv_{surrogate}_{strategy}"
                DatasetCatalog.register(
                    name, lambda x=image_dir, y=gt_dir: load_sem_seg(y, x, gt_ext="png", image_ext="pt")
                )
                MetadataCatalog.get(name).set(
                    stuff_classes=PCONTEXT_SEM_SEG_CATEGORIES[:],
                    image_root=image_dir,
...
register_all_pcontext_adv(_root)
