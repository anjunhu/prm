...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        # self.transformer = TransformerVit(width, layers, heads, layermaskvit = layermaskvit, maskselfattn = maskselfattn)
    ...
    def forward(
        ...
        **kwargs
        ...
        x = self.conv1(x)  # shape = [*, 768, gh, gw]
        b, _, gh, gw = x.size()
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, 768, grid_h, grid_w]
        x = x.permute(0, 2, 1)  # shape = [*, grid_h*grid_w, width]
        x = torch.cat(
            [
        ...
        x,intermediate_feature = self.transformer(x)
        if return_attention:
            return x
