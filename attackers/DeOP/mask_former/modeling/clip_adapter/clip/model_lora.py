...
class ResidualAttentionBlockVit(nn.Module):
    ...
    outlayer = False,maskselfattn = False, layerVit = -1):
        ...
        self.mlp = nn.Sequential(
            OrderedDict(
                [
                    ("c_fc", nn.Linear(d_model, d_model * 4, 16)),
                    ("gelu", QuickGELU()),
                    ("c_proj", nn.Linear(d_model * 4, d_model, 16)),
                ]
            )
        )
        self.ln_2 = LayerNorm(d_model)
...
class MultiheadAttentionLoRA(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
                 kdim=None, vdim=None, batch_first=False, device=None, dtype=None, r=16) -> None:
        ...
        if not self._qkv_same_embed_dim:
            ...
            self.in_proj_lora_A = nn.Parameter(torch.zeros((r, embed_dim)))
            self.in_proj_lora_B = nn.Parameter(torch.zeros((3 * embed_dim, r)))
        self.lora_scaling = 1./r
        ...
        self.out_proj_lora_A = nn.Parameter(torch.zeros((r, embed_dim)))
        self.out_proj_lora_B = nn.Parameter(torch.zeros((embed_dim, r)))
    ...
    def forward(
        ...
        attn_output, attn_output_weights = multi_head_attention_forward_lora(
                ...
                is_causal=is_causal, 
                in_proj_lora_A=self.in_proj_lora_A, in_proj_lora_B=self.in_proj_lora_B,
                out_proj_lora_A=self.out_proj_lora_A, out_proj_lora_B=self.out_proj_lora_B,
...
class ResidualAttentionBlockVitPosition(nn.Module):
    def __init__(self, d_model: int, n_head: int,vision_width,learn_position = False,\
        ...
        self.layerVit = -1
        if outlayer:
            self.attn = MultiheadAttentionVitLoRA(d_model, n_head, maskselfattn=maskselfattn, layerVit=layerVit)
            self.layerVit = layerVit
        else:
            self.attn = MultiheadAttentionLoRA(d_model, n_head)
        self.ln_1 = LayerNorm(d_model)
        self.mlp = nn.Sequential(
            OrderedDict(
                [
                    ("c_fc", lora.Linear(d_model, d_model * 4, r=16, merge_weights=False)), ###!
                    ("gelu", QuickGELU()),
                    ("c_proj", lora.Linear(d_model * 4, d_model, r=16, merge_weights=False)), ###!
                ]
            )
        )
        self.ln_2 = LayerNorm(d_model)
...
def lora_linear(input: Tensor, weight: Tensor,
                lora_A: Tensor, lora_B: Tensor, 
                bias: Optional[Tensor] = None, 
                scaling: float= 1/16.) -> Tensor:
    r"""
    Applies a linear transformation to the incoming data: :math:`y = xA^T + b`.
    This operator supports :ref:`TensorFloat32<tf32_on_ampere>`.
    Shape:
        ...
        - Output: :math:`(N, *, out\_features)`
    """
    result = torch._C._nn.linear(input, weight, bias) 
    # print(input.shape, result.shape)
    result += (input @ lora_A.transpose(0, 1) @ lora_B.transpose(0, 1)) * scaling
    return result
...
class MultiheadAttentionVitLoRA(nn.Module):
    bias_k: Optional[torch.Tensor]
    bias_v: Optional[torch.Tensor]
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None,
                 maskselfattn=False, layerVit = -1, r=16):
        super(MultiheadAttentionVitLoRA, self).__init__()
        self.embed_dim = embed_dim
        self.kdim = kdim if kdim is not None else embed_dim
        self.vdim = vdim if vdim is not None else embed_dim
        self._qkv_same_embed_dim = self.kdim == embed_dim and self.vdim == embed_dim
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, key_padding_mask: Optional[Tensor] = None,
                ...
                mask_feature : Optional[Tensor] = None, maskselfattnsoftmax = False,\
                vitL = False) -> Tuple[Tensor, Optional[Tensor]]:
        return multi_head_attention_forward_vit_softmax_noattn_lora(
                query, key, value, self.embed_dim, self.num_heads,
                self.in_proj_weight, self.in_proj_bias,
                self.bias_k, self.bias_v, self.add_zero_attn,
                self.dropout, self.out_proj.weight, self.out_proj.bias,
...
def multi_head_attention_forward_lora(
    ...
    in_proj_lora_A: Optional[Tensor] = None,
    in_proj_lora_B: Optional[Tensor] = None,
    out_proj_lora_A: Optional[Tensor] = None,
    out_proj_lora_B: Optional[Tensor] = None,
    lora_scaling: float=1/16.,
    mask_feature: Optional[Tensor] = None,
    learn_token: bool=False,
    maskselfattnsoftmax = False, 
    layerVit = -1
    ...
    if not use_separate_proj_weight:
        if (query is key or torch.equal(query, key)) and (key is value or torch.equal(key, value)):
            # self-attention
            q, k, v = lora_linear(query, in_proj_weight, in_proj_lora_A, in_proj_lora_B, in_proj_bias, lora_scaling).chunk(3, dim=-1)
    q = q * scaling
    q = q.contiguous().view(tgt_len, bsz * num_heads, head_dim).transpose(0, 1)
    if k is not None:
        k = k.contiguous().view(-1, bsz * num_heads, head_dim).transpose(0, 1)
    ...
    attn_output = lora_linear(attn_output, out_proj_weight, out_proj_lora_A, out_proj_lora_B, out_proj_bias, lora_scaling)
...
def multi_head_attention_forward_vit_softmax_noattn_lora(
    query: Tensor,
    key: Tensor,
    value: Tensor,
    embed_dim_to_check: int,
