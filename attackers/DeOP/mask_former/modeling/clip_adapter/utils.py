...
def build_clip_model(model: str,maskselfattn=False, layermaskvit=[11],learn_token=False, prompt_num_token=5,learn_position=False,position_layers=[],input_resolution_run:int=384, asBackbone: bool = False, frozen: bool = True):
    rank = get_local_rank()
    
    if rank == 0:
        # download on rank 0 only
        model, _ = clip.load(model,maskselfattn,layermaskvit,learn_token, prompt_num_token,learn_position,position_layers,input_resolution_run, asBackbone, device="cpu")
    synchronize()
    if rank != 0:
        model, _ = clip.load(model,maskselfattn,layermaskvit, learn_token, prompt_num_token,learn_position,position_layers,input_resolution_run,asBackbone, device="cpu")
    synchronize()
    ...
    if frozen:
        for name, param in model.named_parameters():
            # import pdb; pdb.set_trace()
            if not (name.endswith("learned_position") or name.endswith("deep_prompt_embeddings")):
