...
class TransformerDecoderLinear(nn.Module):
    ...
    def forward(
        ...
        intermediate = []
        intermediate_attn_output_weights = []
        ...
        for layer in self.layers:
            ...
            if self.return_intermediate:
                intermediate.append(self.norm(output))
                intermediate_attn_output_weights.append(attn_output_weights)
        outputl, attn_output_weights=self.outlayer(
            output,
            memory,
            tgt_mask=tgt_mask,
            memory_mask=memory_mask,
        ...
        if self.return_intermediate:
            return torch.stack(intermediate), intermediate_attn_output_weights
        
        return outputl.unsqueeze(0), attn_output_weights.unsqueeze(0)
...
class TransformerDecoderLayerLinear(nn.Module):
    ...
    def forward_post(
        ...
        tgt = self.norm1(tgt)
        if self.mask_feature:
            tgt2, attn_output_weights = self.multihead_attn_my(
                query=self.with_pos_embed(tgt, query_pos),
                key=self.with_pos_embed(memory, pos),
                value=memory,
                attn_mask=memory_mask,
...
class TransformerDecoderOutLayerLinear(nn.Module):
    ...
    def forward_post(
        ...
        if need_weights:
            _, tgt2, attn_output_weights = self.multihead_attn_my(
                query=self.with_pos_embed(tgt, query_pos),
                key=self.with_pos_embed(memory, pos),
                value=memory,
                attn_mask=memory_mask,
            ...
            return tgt2, attn_output_weights
        else:
            tgt2, _, attn_output_weights = self.multihead_attn_my(
                query=self.with_pos_embed(tgt, query_pos),
                key=self.with_pos_embed(memory, pos),
                value=memory,
                attn_mask=memory_mask,
...
class MultiHeadAttentionMy(nn.Module):
    ...
    def forward(self,
        ...
        # torch.Size([100, 1, 512]) torch.Size([1064, 1, 512]) torch.Size([1064, 1, 512])  
        ...
        # q [8, 100, 64]) k [8, HW 64] v [8, HW, 64]

        src_len = k.size(1)

        attn_output_weights = torch.bmm(q, k.transpose(1, 2)) # [8, 100, HW] (batch, num_heads, L, S)


        # proposal mask 处理
        mask_shape = proposal_mask.shape # [1, 100, H, W]
        ...
        mask_nhead = mask_list_stack.view(mask_stack_shape[0] * mask_stack_shape[1], \
            mask_stack_shape[2], mask_stack_shape[3]) # [8, 100, HW]
        ...
        attn_output = torch.bmm(attn_output_weights, v) # [8, 100, HW] % [8, HW, 64] = [8, 100, 64]

        attn_output = attn_output.transpose(0, 1).contiguous().view(tgt_len, bsz, embed_dim)
        # print(attn_output.shape) # [100, 1, 512]
        # import pdb; pdb.set_trace()
        if not self.outDecoderLayer:
            attn_output = linear(attn_output, self.out_proj.weight, self.out_proj.bias)
        # print(attn_output.shape) # [100, 1, 512] (L,N,E)

        if need_weights:
            # average attention weights over heads
            attn_output_weights = attn_output_weights.view(bsz, self.num_heads, tgt_len, src_len)
            return attn_output, [attn_output_weights.sum(dim=1) / self.num_heads,attn_output], attn_output_weights
        else:
