...
class ZeroShotClipfeatUpsample_addnorm_vit16Query2D_maskvit(MaskFormer):
    ...
    def forward(self, batched_inputs):
        ...
        if "text_features" in batched_inputs[0]:
            text_features = batched_inputs[0]["text_features"]
        elif "vocabulary" in batched_inputs[0]:
            class_names = batched_inputs[0]["vocabulary"]
    ...
    def preprocess(self, batched_inputs):
        ...
        outputs = {}
        outputs["image_tensor"] = images.tensor
        return outputs

    def get_clip_features(self, batched_inputs):
        dataset_name = [x["meta"]["dataset_name"] for x in batched_inputs]
        assert len(set(dataset_name)) == 1
        dataset_name = dataset_name[0]
        ...
        semseg_pred_mask = outputs["pred_masks"]
        # gt_masks = torch.stack([x["instances"].gt_masks for x in batched_inputs]).float().to(outputs["pred_masks"].device)
        # semseg_pred_mask = F.interpolate(gt_masks, size=outputs["pred_masks"].shape[-2:])
        # print(outputs["pred_masks"].shape, semseg_pred_mask.shape)
        
        if self.maskselfattn:
            clip_images_features, intermediate_features = self.clip_adapter.clip_model.visual(images.tensor, return_cls = False,\
            mask_feature=semseg_pred_mask, maskselfattnsoftmax = self.maskselfattnsoftmax, ps_shortcut = self.ps_shortcut)
        else:
            clip_images_features, intermediate_features = self.clip_adapter.clip_model.visual(images.tensor, return_cls = False,\
            mask_feature=semseg_pred_mask, ps_shortcut = self.ps_shortcut)
