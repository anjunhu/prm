...
def register_pascal_context_459(root):
    root = os.path.join(root, "pcontext-459")
    meta = _get_voc_meta(PASCALCONTEX459_NAMES)
    for name, image_dirname, sem_seg_dirname in [
        ("val", "JPEGImages", "pc459_val"),
    ]:
...
def register_all_pcontext_adv(root):
    root = os.path.join(root, "pcontext")
    for strategy in ['pgd', 'baseline', 'classes_in_scene_with_clip_image_losses']:
        for name, dirname in [("train", "train"), ("val", "val")]:
            image_dir = os.path.join(root, dirname, f"image_adv_san_{strategy}")
            gt_dir = os.path.join(root, dirname, "label")
            name = f"pcontext_sem_seg_{name}_adv_san_{strategy}"
            DatasetCatalog.register(
