...
def register_pascal_context_59(root):
    ...
    for name, image_dirname, sem_seg_dirname in [
        ...
        ("train", "JPEGImages", "annotations_detectron2/pc59_train"),
    ]:
...
STRATEGIES = {'san': ['pgd', 'latest', 'intermediate_features_only'],
              'clip': ['VitB16_intermediate_features', 'VitL14_intermediate_features'],
              'deop': ['pgd', 'latest', 'pgd_ce', 'pgd_decoder_dice', 'intermediate_features_attn_output',
                       'intermediate_features_only', 'intermediate_features_only_400', 'no_ps',
                       'intermediate_features_only_gps', 'intermediate_features_only_mps', 'intermediate_features_only_no_learned_pos'],
              'cat': ['pgd', 'latest'],
              'simseg': ['pgd', 'latest'],
              'zegformer': ['pgd', 'latest'],
              }

def register_pascal_context_59_adv(root):
    root = os.path.join(root, "VOCdevkit", "VOC2010")
    meta = _get_pascal_context_59_meta()

    for surrogate in STRATEGIES.keys():
        for strategy in STRATEGIES[surrogate]:
            for name, image_dirname, sem_seg_dirname in [
                ("val", f"JPEGImages_adv_{surrogate}_{strategy}", "annotations_detectron2/pc59_val"),
            ]:
                image_dir = os.path.join(root, image_dirname)
                gt_dir = os.path.join(root, sem_seg_dirname)
                name = f"context_59_{name}_sem_seg_{surrogate}_{strategy}"
                DatasetCatalog.register(name, lambda x=image_dir, y=gt_dir: load_sem_seg(y, x, gt_ext='png', image_ext='pt'))
                MetadataCatalog.get(name).set(image_root=image_dir, seg_seg_root=gt_dir, evaluator_type="sem_seg", ignore_label=255, **meta,)

def register_pascal_context_459_adv(root):
    root = os.path.join(root, "VOCdevkit", "VOC2010")
    meta = _get_pascal_context_459_meta()

    for surrogate in ['san', 'cat', 'deop', 'simseg', 'zegformer']:
        for strategy in STRATEGIES[surrogate]:
            for name, image_dirname, sem_seg_dirname in [
                ("val", f"JPEGImages_adv_{surrogate}_{strategy}", "annotations_detectron2/pc459_val"),
            ]:
                image_dir = os.path.join(root, image_dirname)
                gt_dir = os.path.join(root, sem_seg_dirname)
                name = f"context_459_{name}_sem_seg_{surrogate}_{strategy}"
                DatasetCatalog.register(name, lambda x=image_dir, y=gt_dir: load_sem_seg(y, x, gt_ext='tif', image_ext='pt'))
                MetadataCatalog.get(name).set(image_root=image_dir, seg_seg_root=gt_dir, evaluator_type="sem_seg", ignore_label=255, **meta,)
...
def register_pascal_context_459(root):
    ...
    for name, image_dirname, sem_seg_dirname in [
        ...
        ("train", "JPEGImages", "annotations_detectron2/pc459_train"),
    ]:
        image_dir = os.path.join(root, image_dirname)
        gt_dir = os.path.join(root, sem_seg_dirname)
...
register_pascal_context_59_adv(_root)
register_pascal_context_459_adv(_root)